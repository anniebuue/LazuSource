...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	var textBoxTypes:Array<String> = ['finana', 'elira', 'pomu'];
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'sheesh':
				boxType = 'speech_sheesh';
			case 'diamond-city-lights':
				boxType = 'speech_DCL';
		}

		box = new FlxSprite(0, 0);
		box.frames = Paths.getSparrowAtlas(boxType);
		box.scrollFactor.set();
		box.antialiasing = ClientPrefs.globalAntialiasing;
		box.animation.addByPrefix('idle', 'dialogue_default', 24);
		box.animation.addByPrefix('pomu', 'dialogue_pomu', 24);
		box.animation.addByPrefix('finana', 'dialogue_finana', 24);
	...
	function startNextDialog():Void
	{
		...
		var charArray = ['elira', 'finana', 'pomu'];

		if(character != lastCharacter) {
			var defaultAnim = 'idle';
			var charThing = arrayCharacters[character].curCharacter;

			for (char in charArray)
			{
				if (charThing.toLowerCase().contains(char))
				{
					defaultAnim = char;
				}
			}

			box.animation.play(defaultAnim, true);
			updateBoxOffsets(box);
		}
