...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.image('mainmenuDesat'));
	...
	override function update(elapsed:Float)
	{
		...
		if (iconThing != null)
		{
			iconThing.scale.set(FlxMath.lerp(1, iconThing.scale.x, CoolUtil.boundTo(1 - (elapsed * 3.125), 0, 1)), FlxMath.lerp(1, iconThing.scale.y, CoolUtil.boundTo(1 - (elapsed * 3.125), 0, 1)));
		}
	...
	override function beatHit()
	{
		...
		iconThing.scale.x += 0.030;
		iconThing.scale.y += 0.030;
	}
	...
	function changeItem(huh:Int = 0)
	{
		...
		var prevScale = FlxPoint.get(iconThing.scale.x, iconThing.scale.y);

		iconThing.destroy();
		iconThing = new FlxSprite().loadGraphic(Paths.image('mainmenu/whatthefart/${optionShit[curSelected]}'));
		iconThing.scrollFactor.set();
		iconThing.screenCenter(Y);
		iconThing.x = 330 - (iconThing.width / 2);
		iconThing.scale.set(prevScale.x, prevScale.y);
		add(iconThing);
